...
def _http_call_fund_analysis(question: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Fund analysis API request failed: {message}")
    except urllib_error.URLError as exc:
        raise RuntimeError(f"Fund analysis API request failed: {exc.reason}")
...
async def diagnose_fund(
    ...
    if save_to_file and result["content"] and _has_valid_display_data(raw):
        ...
        output_path = out_dir / f"fund_diagnosis_{unique_suffix}.md"
