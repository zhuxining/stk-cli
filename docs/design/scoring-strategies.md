...
`scan-live` 是独立于日线 `scan` 的提醒模式。它不改变 `decision.signal` 和 `decision.strength`，而是先用完整日线信号做底层过滤，再用完整分钟 K 线判断盘中触发。

输入数据：

- 日线背景来自 `calc_score()`，因此仍遵守完整日线口径。
- 分钟 K 线使用未缓存的 Longport K 线数据，支持 `5m` 和 `15m`。
- 最新未完成分钟 K 线会被剔除；实时价只作为展示字段。

输出信号：

| live_signal | 判定规则 |
|-------------|----------|
| `实时跟随` | 日线为 `趋势买入` 或 `超卖修复`，分钟收盘站上 VWAP 和 EMA20。 |
| `实时转弱` | 日线偏多但分钟线跌破 VWAP/EMA20 或开盘区间低点；或日线为 `趋势退出` 且分钟线继续弱势。 |
| `实时过热` | 日线偏多，但分钟 RSI14 或相对 VWAP 偏离过热。 |

提醒强度：

- `强提醒`：分钟触发伴随最新完整分钟 K 成交额达到最近分钟均值的 1.5 倍以上，或过热程度达到更高阈值。
- `普通提醒`：基础实盘触发成立，但量能或过热程度未达到强提醒。
- `观察`：日线背景存在但分钟线未收线触发，默认不进入 `focus`。
