...
- 扫描输出默认只为 `priority=high` 标的附带最近 10 根压缩日线，其他标的只保留决策、上下文和风控摘要。
...
| 每日扫描输出什么？ | 输出 `MonitorResult`，只展开 `focus` 重点关注标的，并仅为 `priority=high` 标的补充最近 10 根压缩日线 | 输出所有股票的完整分析明细，或为所有 `focus` 标的附带完整 K 线 | 用户核心场景是从 100 个以上股票中找出少数有效信号；默认过滤无信号标的并限制 K 线明细能降低 Agent 上下文成本 | 需要审计完整股票池时新增显式全量输出命令 |
...
- **Focus Item**：一次 Monitor Run 中被筛选进入重点关注列表的单个标的，可在高优先级时携带压缩日线解释数据。
...
stk stock scan / stk watchlist scan
    ...
    -> Scan Service supplements priority=high focus items with compact daily10 rows
...
- `services/scan.py` 负责把多个 `ScoreResult` 聚合为 `MonitorResult`，并按重点关注规则生成 `focus`、`summary`、`ignored` 和 `errors`；仅为 `priority=high` 的 `FocusItem` 补充最近 10 根压缩日线。
