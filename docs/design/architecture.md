...
| 每日扫描输出什么？ | 输出 `MonitorResult`，只展开 `focus` 重点关注标的，并保留覆盖统计与错误列表 | 输出所有股票的完整分析明细 | 用户核心场景是从 100 个以上股票中找出少数有效信号，默认过滤无信号标的能降低自动化后处理成本 | 需要审计完整股票池时新增显式全量输出命令 |
...
- **Monitoring Decision**：面向每日监控的动作判断，包含 `focus_buy`、`focus_sell` 或 `watch`，并携带信号强度、置信度和新鲜度。
- **Signal Context**：非主策略指标形成的辅助因子集合，用于解释主信号质量、冲突、风险或左侧机会。
- **Risk Profile**：基于 ATR 与 Supertrend 的止损、止盈、风险收益比和风险等级。
- **Monitor Run**：一次 watchlist 或临时股票池扫描，包含股票池覆盖情况、重点关注列表、忽略统计和失败列表。
- **Focus Item**：一次 Monitor Run 中被筛选进入重点关注列表的单个标的。
...
stk stock scan / stk watchlist scan
    ...
    -> Scan Service filters active focus candidates and sorts by priority, context bias, confidence and signal age
...
- `services/scan.py` 负责把多个 `ScoreResult` 聚合为 `MonitorResult`，并按重点关注规则生成 `focus`、`summary`、`ignored` 和 `errors`。
...
| `services/indicator.py` | 生成按日合并的 OHLCV、EMA9/26、Supertrend 与其他技术指标结果 | 判断重点关注标的 |
| `services/score.py` | 基于日线 K 线生成 `ScoreResult`，包含决策、主信号、辅助因子和风控点位 | 管理自选股或批量排序 |
| `services/scan.py` | 聚合每日监控结果，输出重点关注标的、统计、忽略数量和错误列表 | 渲染响应或计算单标的指标 |
